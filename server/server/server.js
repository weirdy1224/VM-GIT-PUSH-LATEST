...
// Extract section items (Impact, Remediation, POC)
function extractSectionItems(text, regex, isPOC = false) {
  ...
  // Split by lines, handle both numbered and bulleted items
...
app.post("/api/save-report", upload.single("file"), async (req, res) => {
  try {
    ...
    try {
      ...
      parsedVulnerabilities.forEach((vuln, index) => {
        if (!vuln.bug_name || typeof vuln.bug_name !== "string") {
          throw new Error(`Vulnerability[${index}]: bug_name must be a string`);
        }
