  ...
  useEffect(() => {
    const fetchScores = async () => {
      try {
        ...
        const response = await axios.get("http://localhost:5000/api/cvss-scores", {
        ...
        if (response.data.success) {
          ...
          if (Array.isArray(response.data.data)) {
            fetchedScores = response.data.data
              ...
              .filter((score) => !isNaN(score.cvss_score) && score.cvss_score >= 0 && score.cvss_score <= 10);
          }
    ...
    <div className="space-y-2">
      {filteredScores.map((score, index) => (
        <div key={index} className="flex items-center">
          <div className="w-12 text-sm" style={{ color: "var(--text-secondary)" }}>
            {score.range}
          </div>
          <div className="flex-1 h-6 flex items-center">
            {score.value > 0 && (
              <div className="flex items-center w-full">
                <div
                  className={`h-1.5 rounded-full ${getColor(score.range)} transition-all duration-500`}
                  style={{ width: `${Math.min((score.value / referenceMax) * 100, 100)}%` }}
                />
                <span
        ...
        <div className="graph-overlay">
          <div className="overlay-content">
            ...
            <div className="overlay-details">
              <h3>CVSS Score Range</h3>
              <p>
                This chart displays the distribution of CVSS (Common Vulnerability Scoring System) scores for vulnerabilities within the selected date range.
              </p>
              <p>
                <strong>Key Insights:</strong>
              </p>
              <ul>
                <li><strong>0-1 (Green):</strong> Low-risk vulnerabilities, minimal impact.</li>
                <li><strong>1-3 (Yellow):</strong> Moderate risk, may require attention.</li>
                <li><strong>3-5 (Orange):</strong> Elevated risk, prioritize mitigation.</li>
                <li><strong>5-7 (Red):</strong> High risk, immediate action recommended.</li>
                <li><strong>7+ (Dark Red):</strong> Critical vulnerabilities, urgent remediation needed.</li>
              </ul>
              <p>
                Use this chart to assess the severity of vulnerabilities in your system and prioritize security efforts accordingly.
              </p>
